...
if ($result->num_rows > 0) {
  ...
  $sql_createarchive = "INSERT INTO grant_archive (grantid, jsondata, creation_date)
  ...
  if ($con->query($sql_createarchive) === TRUE) {
    ...
    $sql_updatecurrentdata = "UPDATE grants SET jsondata='$jsondata' WHERE id=" . $_SESSION['current_grant'];
    $sql_updatecreationdate = "UPDATE grants SET creation_date=now() WHERE id=" . $_SESSION['current_grant'];

    if ($con->query($sql_updatecurrentdata) === TRUE && $con->query($sql_updatecreationdate) === TRUE)
    {

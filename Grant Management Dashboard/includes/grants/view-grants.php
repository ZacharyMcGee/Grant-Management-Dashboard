...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<a href='#' onClick='openGrant(" . $row["id"] . ")'><div class='full-card-left-border-active'><div class='view-grants-chart'><canvas id='chart" . $i . "'></canvas></div><div class='view-grants-info'><span class='view-grants-title'>" . $row["name"] . " </span><div class='view-grants-bp'> #" . $row["bp"] . "</div><br><span class='view-grants-update'>" . $timeSinceUpdate . "</span><br><span class='view-grants-award'>DC Award: " . $formattedAward . "</span></div><div class='view-grants-buttons'><button class='button'>Manage</button><button class='button'>Report</button><button class='button'>Delete</button></div></div></a><script>moneyLeftPieChart('chart" . $i . "','" . $row["dc_award"] . "','" . $row["jsondata"] . "');</script>";
        $i++;
    }
}
else
